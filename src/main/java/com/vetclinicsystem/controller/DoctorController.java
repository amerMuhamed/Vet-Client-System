...
public class DoctorController {
    ...
    public ResponseEntity<ApiResponse<DoctorDto>> assignDoctorToClinic(
            @PathVariable Long doctorId,
            @PathVariable Long clinicId
    ) {
        Doctor updatedDoctor = doctorService.assignDoctorToClinic(doctorId, clinicId);
        return ResponseEntity.ok(ApiResponse.success(
                DoctorMapper.toDto(updatedDoctor), "Doctor assigned to clinic successfully"));
    }

    @PutMapping("/{doctorId}/deassign")
    public ResponseEntity<ApiResponse<DoctorDto>> deAssignDoctorFromClinic(
            @PathVariable Long doctorId
    ) {
        Doctor updatedDoctor = doctorService.deAssignDoctorFromClinic(doctorId);
        return ResponseEntity.ok(ApiResponse.success(
                DoctorMapper.toDto(updatedDoctor), "Doctor de-assigned from clinic successfully"));
    }

    @PostMapping("/{doctorId}/profile-image")
    public ResponseEntity<ApiResponse<DoctorDto>> uploadProfileImage(
            @PathVariable Long doctorId,
            @RequestParam("file") MultipartFile file
    ) throws IOException {
        Doctor doctor = doctorService.assignProfileImage(doctorId, file);
        return ResponseEntity.ok(ApiResponse.success(
                DoctorMapper.toDto(doctor), "Profile image uploaded successfully"));
    }
